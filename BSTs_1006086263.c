...
BST_Node *newBST_Node(double freq, int bar, double index)
{
    ...
    if (new_node == NULL){
        printf("new_BST_Node(): Error, there is not enough memory to create new node, returning NULL\n");
        return NULL;
    } 
...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
    ...
    double key_target = (10*bar) + index;

    if (key_target == root->key){
        return root;
    }
...
void storeInOrder(BST_Node *root, BST_Node **arr, int *i)
{
    if (root == NULL)
        return;
...
void findBarRange(BST_Node *root, int *min_bar, int *max_bar)
{
    if (root == NULL)
        return;
    ...
    findBarRange(root->left, min_bar, max_bar);
    findBarRange(root->right, min_bar, max_bar);
}
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    int min_bar = root->bar;
    int max_bar = root->bar;

    findBarRange(root, &min_bar, &max_bar);

    int n = countNodes(root);

    BST_Node **arr = calloc(n, sizeof(BST_Node *));
    if (arr == NULL)
        return root;
    ...
    for (int j=0; j<n; j++)
    {
        ...
        node->bar = max_bar + min_bar - node->bar;
        node->index = 1 - node->index;
        node->key = (10 * node->bar) + node->index;
    }
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    for (j = 0; j < n; j++)
    {
        ...
        int freq_index = findFreqIndex(node->freq);

        if (freq_index != -1)
        {
            int new_index = freq_index + semitones;

            if (new_index >= 0 && new_index <= 99)
            {
                double new_freq = note_freq[new_index];

                double new_time = node->index + time_shift;

                if (new_time >= 0 && new_time < 1)
                {
                    BST_Node *new_node = newBST_Node(new_freq, node->bar, new_time);
